...
--Создание и заполнение таблиц DWH
--1. Справочник адресов
-- Таблица addresses
CREATE TABLE IF NOT EXISTS clickhouse.addresses (
    address_id UInt64,
...
-- Заполнение таблицы addresses из таблицы customers
INSERT INTO clickhouse.addresses
SELECT
    cityHash64(
...
-- Заполнение таблицы addresses из таблицы stores
INSERT INTO clickhouse.addresses
SELECT
    cityHash64(
...
CREATE TABLE IF NOT EXISTS clickhouse.categories (
    category_id UInt64,
...
-- Заполнение таблицы categories из таблицы products
...
CREATE TABLE IF NOT EXISTS clickhouse.manufacturers (
    inn String,
...
-- Заполнение таблицы manufacturers из таблицы products
INSERT INTO clickhouse.manufacturers
SELECT
...
CREATE TABLE IF NOT EXISTS clickhouse.store_networks (
...
-- Заполнение таблицы store_networks из таблицы stores
INSERT INTO clickhouse.store_networks
SELECT
...
CREATE TABLE IF NOT EXISTS clickhouse.store_managers (
    manager_id UInt64,
    manager_name String,
...
-- Заполнение таблицы store_managers из таблицы stores
INSERT INTO clickhouse.store_managers
SELECT
...
CREATE TABLE IF NOT EXISTS clickhouse.dim_customers (
    customer_id String,
    first_name String,
    ...
    address_id Nullable(UInt64), -- из таблицы addresses
...
-- Заполнение таблицы dim_customers из таблиц customers и addresses
INSERT INTO clickhouse.dim_customers
SELECT
...
CREATE TABLE IF NOT EXISTS clickhouse.dim_products (
    product_id String,
    name String,
...
-- Заполнение таблицы dim_products из таблиц products, manufacturers и categories
...
CREATE TABLE IF NOT EXISTS clickhouse.dim_stores (
    store_id String,
    store_name String,
...
-- Заполнение таблицы dim_stores из таблиц stores, store_networks, store_managers и addresses
INSERT INTO clickhouse.dim_stores
SELECT
...
LEFT JOIN clickhouse.store_networks AS sn ON 
	nullIf(lowerUTF8(trim(s.store_network)), '') = sn.store_network_name
...
CREATE TABLE IF NOT EXISTS clickhouse.store_categories (
    store_id String,
    category_id UInt64,
    event_time DateTime
) ENGINE = ReplacingMergeTree(event_time)
...
-- Заполнение таблицы связи многие-ко-многим между таблицами stores и categories
...
CREATE TABLE IF NOT EXISTS clickhouse.fact_purchases (
    purchase_id String,
    customer_id String,
    store_id String,
...
-- Заполнение таблицы fact_purchases из таблиц purchases и addresses
...
WHERE
    ...
    nullIf(lowerUTF8(trim(p.customer_customer_id)), '') IS NOT NULL AND  			  -- customer_id не может быть NULL
    nullIf(lowerUTF8(trim(p.store_store_id)), '') IS NOT NULL AND     				  -- store_id не может быть NULL
    toDecimal64OrNull(nullIf(trim(p.total_amount), ''), 2) IS NOT NULL AND 			  -- total_amount  не может быть NULL
    parseDateTimeBestEffortOrNull(nullIf(trim(p.purchase_datetime), '')) IS NOT NULL; -- дата покупки не может быть NULL


--11. Таблица фактов деталей покупок fact_purchase_items
-- Дедупликация данных по purchase_id и product_id
CREATE TABLE IF NOT EXISTS clickhouse.fact_purchase_items (
    purchase_id String,
    product_id String,
...
-- Заполнение таблицы fact_purchase_items из таблицы purchases
INSERT INTO clickhouse.fact_purchase_items
SELECT
...
CREATE TABLE IF NOT EXISTS clickhouse.duplicate_analysis_results (
    event_time DateTime,
